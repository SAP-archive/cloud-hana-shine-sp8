...
function deletePO() {
    ...
    conn.prepareStatement("SET SCHEMA \"_SYS_BIC\"").execute(); // Setting the SCHEMA
    var pstmt;
    var rs;
    var query;
    ...
    try {
        ...
        query = 'SELECT LIFECYCLESTATUS, APPROVALSTATUS, CONFIRMSTATUS, ORDERINGSTATUS, INVOICINGSTATUS ' + 'from "{{PACKAGE_NAME}}.data::EPM.PO.Header" where PURCHASEORDERID = ?';
    ...
    try {
        ...
        query = "UPDATE \"{{PACKAGE_NAME}}.data::EPM.PO.Header\" set LIFECYCLESTATUS = 'X' where PURCHASEORDERID = ?";
        pstmt = conn.prepareStatement(query);
        pstmt.setString(1, purchaseOrderID);
        pstmt.executeUpdate();
        pstmt.close();
...
function approvePO() {
    ...
    try {
        ...
        // Update Purchase Order Status in order to delete it
        if (action === "Reject") {
            query = "UPDATE \"{{PACKAGE_NAME}}.data::EPM.PO.Header\" set APPROVALSTATUS = 'R' where PURCHASEORDERID = ?";
        }

        if (action === "Accept") {
            query = "UPDATE \"{{PACKAGE_NAME}}.data::EPM.PO.Header\" set APPROVALSTATUS = 'A' where PURCHASEORDERID = ?";
        }
