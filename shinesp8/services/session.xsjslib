...
function getSessionId() {
    ...
    if (sessionId === null) {
        ...
        var pstmt;
        var rs;
        var query = 'select "{{PACKAGE_NAME}}.data::sessionId".NEXTVAL from dummy';
...
function get_session_variable(name, application) {
    ...
    var rs;
    var output = '';
    var query = 'CALL "{{PACKAGE_NAME}}.procedures::get_session_variable"(?,?,?,?)';
...
function get_session_variables(application) {
    ...
    var cstmt;
    var rs;
    var query = 'CALL "{{PACKAGE_NAME}}.procedures::get_session_variables" (?,?,?)';
    cstmt = conn.prepareCall(query);
    cstmt.setString(1, sessionId);
...
function set_session_variable(name, application, value, expiry) {
    ...
    var conn = $.db.getConnection();
    var cstmt;
    var query = 'CALL "{{PACKAGE_NAME}}.procedures::set_session_variable"(?,?,?,?,?)';
    cstmt = conn.prepareCall(query);
    cstmt.setString(1, sessionId);
    cstmt.setString(2, name);
    cstmt.setString(3, application);
...
function get_application_variable(name, application) {
    ...
    var rs;
    var output = '';
    var query = 'CALL "{{PACKAGE_NAME}}.procedures::get_application_variable"(?,?,?)';
    cstmt = conn.prepareCall(query);
...
function get_application_variables(application) {
    ...
    var cstmt;
    var rs;
    var query = 'CALL "{{PACKAGE_NAME}}.procedures::get_application_variables"(?,?)';
    cstmt = conn.prepareCall(query);
...
function set_application_variable(name, application, value, expiry) {
    ...
    var conn = $.db.getConnection();
    var cstmt;
    var query = 'CALL "{{PACKAGE_NAME}}.procedures::set_application_variable"(?,?,?,?)';
    cstmt = conn.prepareCall(query);
    cstmt.setString(1, name);
    cstmt.setString(2, application);
